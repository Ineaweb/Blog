...
Je vous propose de focusser sur un cas d'utilisation : les agents éphémères.
...
- Un pool de nœud **Windows Server 2019 with container** (pour les **builds** windows),
- Un pool de nœud **Ubuntu Server with container** (pour les **builds** linux), 
...
4. L'**Azure Batch Account** traite les tâches et affecte la tâche au bon pool de nœud,
5. Le pool de nœud instancie un conteneur à partir de l'image présente dans l'**Azure Container Registry**,
...
S'il n'y a pas de disponibilité sur le pool utilisé par le *job**, **Azure Btach Account** va mettre en attente la tâche.
