...
Ce mécanisme vous permet de réaliser vos **builds** et vos déploiements sans induire de biais liés à l'environnement de l'agent (logiciels, variable d'environnement, reliquats d'un précédent **builds**, ...).
...
Enfin, **Azure Batch Account** propose un planificateur de tâche. Mais ce planificateur ne vous permettra de définir que la fréquence d'exécution. Je vous recommande vivement de passer votre chemin et d'utiliser des services intégrants un planificateur complet basé sur un CRONtab par exemple.
