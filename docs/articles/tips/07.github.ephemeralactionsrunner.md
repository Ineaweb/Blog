...
1. **Github** appel la fonction **ProvideActionsRunner** pour demander un **runner** éphémère,
...
Sur **Github** le jeton permettant d'enregistrer notre **runner** à une validité d'une heure. Il faut donc prévoir de demander systématiquement un nouveau jeton pour chaque **job**. Pour cela, il faut appeler l'api **Github**. 
...
Ce qui est super avec **Github** contrairement à **Azure devOps Services**, c'est la multitude de **webhook**.
...
Dans notre cas, nous allons souscrire notre **Azure function** aux **webhook** de **Github**.
...
Après vous avoir montrer qu'il était possible de provisionner ses propres agents **Azure DevOps Services** dans son SI, je réitère ma démonstration sur **Github**. L'usine logicielle change mais le concept fonctionne toujours.
...
- la multitude de **webhook** disponible permettant d'implémenter des scénarii interactifs affectant l'écosystème environnant.
- l'enregistrement des **runner** avec un jeton temporaire qui est bien plus sécurisé que le **Personal Access Token** d'**Azure DevOps**.
