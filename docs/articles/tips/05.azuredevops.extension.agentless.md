...
!!! note
    Vous ne pourrez publier sur le marketplace qu'avec un compte déjà enregistré dans une organisation **Azure DevOps**.

1. Connectez-vous à votre organisation **Azure DevOps** : https://dev.azure.com/[votre organisation]
2. Une fois connectez, générez un **personal acces token** en cliquant sur **User settings** puis sur **Personal access tokens**.

    ![linktopat](../../img/tipsandtricks.05.agentless.004.png)

3. Créer un nouveau **Personal access token** en précisant bien **All accessible organisations** et en cochant **Manage** sous **Marketplace**.

    ![newpat](../../img/tipsandtricks.05.agentless.005.png)

4. Cliquer sur **Create** et bien noter la clé retournée.
5. Executer la commande ci-dessous pour packager l'extension :
   ```bash
    tfx extension create --manifest-globs vss-extension.json
   ```
6. Executer la commande ci-dessous pour publier l'extension sur le marketplace :
   ```bash
    tfx extension publish --manifest-globs vss-extension.json --share-with [liste des organisations avec qui vous voulez partager votre extension (séparé par une virgule) si celle-ci n\'est pas publique]
   ```
7. durant le processus de publication, il vous demandera alors de saisir votre **personal access token**.

Votre extension est maintenant publié. Vous pouvez consulter celle-ci directement sur le [marketplace](https://marketplace.visualstudio.com/azuredevops). Une fois connecté, cliquez sur **Publish extensions**.
...
Dans cet article, j'ai essayé de vous montrer un exemple simple d'extension. Pour cela (et bien que je connaisse bien le système d'extension) j'ai dû mis reprendre de nombreuses fois pour avoir une extension opérationnelle. Les nombreuses limitations sont souvent des facteurs de frustration.

Cependant, les **github action** ne permettant pas aujourd'hui ce type de tâche, nous n'avons pas vraiement le choix avec les outils Microsoft... Il reste encore beaucoup de choses à améliorer de ce point de vue !
