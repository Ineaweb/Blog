...
Vous allez pouvoir "customiser" vos **boards** Azure, la gestion des **repositories** Azure, vos pipelines de **CI/CD**, ...
...
!!! note
    Les tâches **agentless** sont executées depuis les serveurs **Azure Devops** qui ont des plages d'ip bien définit (cf. [documentation microsoft](https://docs.microsoft.com/fr-fr/azure/devops/organizations/security/allow-list-ip-url?view=azure-devops&tabs=IP-V4#inbound-connections)). Elles offrent donc l'avantage d'être plus facile à **whitelister** auprès des **firewall** que les agents **Microsoft hosted**.  
...
Ensuite dans votre répertoire de travaille, vous allez créer 2 dossiers et 3 fichiers comme ceci :
...
- **files** : doit lister tous les dossiers qui devront être intégré dans l'extension,
- **contribution** :
...
- l'attribut **mane** doit correspondre à un **id** du nœud **contributions** dans le fichier **vss-extension.json**, 
- l'attribut **runsOn** doit contenir **Server** et/ou **ServerGate** pour être considéré comme une tâche **agentless**, 
- le nœud **inputs** doit contenir tous les paramètres de la tâches,
...
- **properties** :
  ...
  - **url** : correspond à l'url du service,
...
!!! note
    Les différents types d'authentification sont décrit [ici](https://docs.microsoft.com/fr-fr/azure/devops/extend/develop/auth-schemes?view=azure-devops)
...
- l'attribut **EndpointId** correspond à "$([l'attribut **name** dans le nœud **inputs**])",
...
- les paramètres décrit dans le nœud **inputDescriptors** ne peuvent pas être utilisés si l'attribut **isConfidential** est définit à "true",
- les modes d'authentification permis sont trop peu nombreux. Il aurait été bienvenue de permettre les authentifications modernes comme l'**oauth 2.0** et les authentifications simples comme l'**apikey** ou le **sasToken** dans la **query http**,
...
6. Exécutez la commande ci-dessous pour publier l'extension sur le marketplace :
   ...
   ```
7. durant le processus de publication, il vous demandera alors de saisir votre **personal access token**.

Votre extension est maintenant publiée. Vous pouvez consulter celle-ci directement sur le [marketplace](https://marketplace.visualstudio.com/azuredevops). Une fois connecté, cliquez sur **Publish extensions**.

## Conclusion

Bien que les capacités d'**Azure DevOps** peuvent être étendu sur de nombreux aspects, il s'avère que la documentation Microsoft pour aider les développeurs est souvent éparpillée et/ou incomplète.

Dans cet article, j'ai essayé de vous montrer un exemple simple d'extension. Pour cela, bien que je connaisse le système d'extension, j'ai dû mis reprendre de nombreuses fois pour avoir une extension opérationnelle. Les nombreuses limitations sont souvent des facteurs de frustration.
