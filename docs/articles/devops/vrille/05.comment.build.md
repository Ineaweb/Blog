...
1. si oui, on vérifie que ses dépendances n'ont pas évoluées.
2. si c'est le cas, on passe au composant suivant. Sinon, on déclenche le **build**.
3. si celui-ci n'a pas déjà été correctement buildé, on déclenche le **build**.
4. on attend la fin du **build** et on récupère le résultat.
5. si le **build** est OK, on passe au composant suivant.
6. si le **build** est KO, on stoppe le **build** des composants dépendants.
