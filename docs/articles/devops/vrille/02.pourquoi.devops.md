...
- Le code de nos libraires sera réparti dans des **repository** en fonction de son utilisation. Il n'est pas pertinent de mettre dans un **repository** à part le code d'une libraire qui n'est destinée qu'à un seul microservice.
...
!!! note
    Je ne parle pas ici de **PipelineAsCode**. Pratique que je recommande fortement dans la démarche DevOps. Les **PipelineAsCode** permettent de décrire le fonctionnement de notre pipeline de CI/CD. Personnellement, je recommande de n'avoir qu'un seul pipeline par **repository** et que celui-ci soit déposé à la racine du repository. Cela permet de rapidement comprendre ce qui va être déclenché lors d'un commit-push sur un **repository** sans avoir la mauvaise surprise d'avoir déclenché un pipeline non désiré.
...
En effet, une libraire créée dans le cadre d'un projet et pensée pour être utilisée par plusieurs microservices ou par d'autres libraires aura son propre **repository**.
...
Afin de garantir la robustesse de la solution, je veux m'assurer qu'une évolution sur une librairie ne va pas provoquer une régression sur les libraires et les microservices qui dépendent d'elle.
...
!!! note
    L'utilisation de **feature flag** nous permettra dans certain cas d'éviter un déploiement, mais ce n'est pas pour autant qu'il n'y aura pas de phase dans notre pipeline. Il faudra prévoir à la place du déploiement l'activation ou la désactivation de tel ou tel **feature**.
...
Il a l'intérêt  d'être simple à gérer grâce à son unique branche de développement et compatible avec le maintien  opérationnel de plusieurs versions techniques d'un composant grâce à ses branches de **release**.
...
Ces branches de **release** vont contenir le code "sanctuarisé". Le code contenu dans ces branches a passé l'intégralité des tests et des mesures de qualité et de sécurité imposé par l'équipe. Le code est **stable**.
