...
summary: "Dans cette dernière partie d'un unique chapitre, je vais vous présenter l'implémentation concrète de notre idée."
authors:
    - Philippe Morisseau
date: 2022-02-08
...
Durant la première partie nous avons détaillé comment nous pouvions atteindre les limites du DevOps et faire partir en vrille notre démarche. Ensuite, dans la seconde partie, je vous ai proposer une idée qui permettrait de passer au-delà des limites. 
Dans cette dernière partie d'un unique chapitre, je vais vous présenter l'implémentation concrète de notre idée.

## L'implémentation

### Nos objectifs au travers de l'implémentation de notre idée

Avant de foncer tête baisser dans la réalisation de notre idée, il faut prendre un peu de recul et se poser les bonnes questions. Que doit apporter notre solution ?
...
Il est évident que chaque client, chaque organisation, chaque projet à sa propre organisation. Notre solution ne doit pas imposer une organisation spécifique. Cela irait à l'encontre de l'agilité et de la démarche DevOps.
...
- à un processus de mise en production. Chaque entreprise ayant son propre processus de **delivery** applicatif avec ses propres environnements hors productions, notre solution doit s'adapter à l'organisation et non l'inverse.
- à un **Semantic Versioning**. Notre solution doit autoriser n'importe quel type de **Semantic Versioning** : Majeur.Mineur.Patch, ...  
- à un environnement technique. Chaque SI d'entreprise à son propre écosystème au travers des **OS**, des **framewoks**, des solutions d'hébergement (**Cloud**, **OnPremise**, ...). Notre solution ne doit pas imposer un hébergeur cloud par exemple.
...
**Azure DevOps Services** offre l'avantage de pouvoir intégrer vos propres extensions (ou celles déjà disponible sur le **Marketplace**).

- Vous pouvez réaliser vos propres extensions web pour augmenter les fonctionnalités des interfaces **Azure DevOps** en **react**.
- Vous pouvez développer  en **powershell** ou en **node.js** vos propres extensions de tâches pour augmenter les fonctionnalités de vos pipelines de **CI/CD**.
- Vous pouvez définir vos propres connections de services afin de sécuriser et de simplifier les connexions entre **Azure DevOps** et vos services.  
...
- Une extension **Web** pour simplifier la mise à jour des référentiels de **build** (cf. [Partie 2 - Chapitre 2](05.comment.build.md)) et de déploiement (cf. [Partie 2 - Chapitre 3](06.comment.deploy.md)) en :
    ...
    - sélectionnant les repos devant intégrer,
...
- Une extension **Task** pour simplifier l'orchestration des **builds** et des déploiements dans notre meta-pipeline en :
    ...
    - gérant le **semantic versioning** définit dans notre référentiel de **build**,
...
5. Nom des branches des composants de l'application depuis lesquelles les pipelines seront déclenchés,
6. Indique s'il faut identifier les **commits** qui ont été **cherrypické** pour les exclures de la recherche d'évolutions d'un composant (par exemple, dans le cas du **Trunk Based Development**, les **cherrypick** sont utilisés pour réaliser des **hotfix**),
...
L'onglet de visualisation des **repositories git** permet de visualiser l'ensemble des **repo.** de votre application et d'avoir un aperçu instantanée des composants nécessitant une montée de version. 
...
L'utilisateur a le dernier mot et peut décider ou non d'embarquer le composant en cochant la case et en saisissant un numéro de version.
...
Mais, cela a un moindre impacte pour l'expérimentation. En effet, on modifie rarement les dépendances entre les composants. L'édition des fichiers Yaml (**build** et **release**) permet déjà dans un premier temps de palier à ce manque.
...
De plus l'automatisation réduit les opérations manuelles et donc les risques d'erreur ou d'oubli.
...
- sur l'ensemble des versions de chaque composants à chaque exécution du meta-pipeline à partir du manifeste.
...
!!! warning
    Cela ne veut pas dire que la solution permet de gérer pour vous tout le **rollback** de votre application, et notamment de vos données. Mais si vous faites attention à prendre en compte l'évolutivité de vos modèles de données dans la conception de vos composants avec ou sans outils (entity framework, liquibase, ...) vous ne devriez pas avoir trop de problèmes au moment du **rollback**.

## Conclusion

Pour le moment la solution que nous avons mis en place est encore "verte". Certes, nous l'avons éprouvé longuement auprès d'un client et de nombreuses fois sur des **Proof of Concept**, mais de mon point de vu elle nécessite encore pas mal d'ajustements et d'améliorations pour être mis à la disposition de tous.
Si vous souhaitez participer, contribuer ou réagir, je vous invite à me contacter en privé via **LinkedIn**. Je serais ravi d'échanger avec vous. 
...
La démarche **DevOps** ne s'arrête pas à la mise en place de pipeline **CI/CD**. C'est une démarche qui impactera forcément votre organisation : les développeurs, les architectes, les intégrateurs, les testeurs, les exploitants, mais aussi les représentants métiers (**Product Owner**, **Business Analyst**). Et pour ces derniers le couple Agilité + **DevOps** prends tout son sens.
