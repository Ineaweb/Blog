...
Ce qui veut dire que l'on a un cycle **DevOps** au niveau de notre application. Ce cycle **DevOps** doit pouvoir piloter les **pipelines DevOps** de tous nos composants et librairies. Il doit aussi s'assurer que les conditions (dépendances de build et dépendances de déploiement) sont respectées.
