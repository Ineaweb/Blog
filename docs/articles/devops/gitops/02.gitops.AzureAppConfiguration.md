...
Commençons par le fichiers de paramètres "classiques". Rien de plus simple. Pour chaque clé, vous allez définir la valeur. Par exemple en yaml vous aurez :
...
Et si vous avez des configurations différentes en fonction de vos environnements (ce qui est très probable), je vous conseille de gérer vos configurations dans des fichiers séparés. 
Par exemple, pour vos paramètres "classiques" vous aurez :
...
- un fichier *config.dev.yml* contenant les paramètres de configuration spécifique à l'environnement de *DEV*,
    ```yaml
    TestApp:Settings:Param1: ValueOfParam1forDEV
    ``` 
- un fichier *config.uat.yml* contenant les paramètres de configuration spécifique à l'environnement de *UAT*, 
    ```yaml
    TestApp:Settings:Param1: ValueOfParam1forUAT
    TestApp:Settings:Param2: ValueOfParam2forUAT
    ``` 
- un fichier *config.prd.yml* contenant les paramètres de configuration spécifique à l'environnement de *PROD*.
    ```yaml
    TestApp:Settings:Param1: ValueOfParam1forPRD
    TestApp:Settings:Param2: ValueOfParam2forPRD
    ``` 

Il faudra ensuite prévoir 
...
Contrairement à ce que l'on pourrait croire, l'intégration continue va être utile pour définir :

- la valeur du paramètre "sentinelle",
- packager la configuration afin de pouvoir reproduire cette configuration.
...
variables:
  azureSubscription: 'Onepoint'
  AppConfigurationEndpoint: 'https://pmo-app-configuration.azconfig.io'
