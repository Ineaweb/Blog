...
Vous allez trouver l'intérêt de mettre en place une démarche **GitOps** dès que vous allez devoir gérer une application distribuée. En effet, dans vos paramètres de configuration, vous allez devoir gérer des attributs communs aux composants de votre architecture (comme les chaines de connexion à vos bases de données) et des attributs qui ne sont pas lié au déploiement de vos composants (comme les certificats). Ces paramètres vont devoir être géré indépendamment de vos composants.
...
Microsoft propose pour ce service des SDK disponible dans de nombreux langages : .NET, Java, Python, Javascript, ...
...
Dans cet exemple, nous utilisons l'identité managée pour authentifier notre application auprès du service **Azure App Configuration**. 

Enfin, Il faut donner le rôle **App Configuration Data Reader (RBAC)** au niveau de l'**Azure App Configuration** à l'application **ASP.NET core**.
...
Il faut créer une classe en charge de déclencher le rafraîchissement auprès du **middleware** d'**Azure App Configuration**. Voici un exemple :
